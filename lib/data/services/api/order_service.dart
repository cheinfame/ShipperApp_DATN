...
class OrderService {
  ...
  Future<Map<String, dynamic>> getOrdersByStatus(
    ...
    final url = Uri.parse('$baseUri/order/user/$userId/status?status=$status');
  ...
  Future<Map<String, dynamic>> acceptOrder(
    ...
    try {
      final response = await http.put(
        Uri.parse('$baseUri/order/$orderId/accept'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
      ...
      throw Exception('Failed to accept order: $error');
    }
  }
  ...
  Future<void> startShipping(String token, String orderId) async {
    final url = Uri.parse('$baseUri/order/$orderId/start-shipping');
    final response = await http.put(
      url,
      headers: {
        'Authorization': 'Bearer $token',
